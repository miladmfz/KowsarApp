...
public class Broker_BasketActivity extends AppCompatActivity {
        ...
        broker_dbh.getAllPreFactorRowsAsync(
                ...
                new Broker_DBH.DbCallback<ArrayList<Good>>() {
                    ...
                    public void onResult(ArrayList<Good> result) {
                        ...
                        broker_basketItemAdapter =
                                new Broker_BasketItemAdapter(goods, Broker_BasketActivity.this);
                        ...
                        gridLayoutManager =
                                new GridLayoutManager(Broker_BasketActivity.this, 1);
                        ...
                        try {
                            binding.bBasketAR1.scrollToPosition(
                                    Integer.parseInt(callMethod.ReadString("BasketItemView")) - 1
                            );
                        } catch (Exception e) {
                            ...
                            public void onScrolled(@NotNull RecyclerView recyclerView, int dx, int dy) {
                                if (dy > 0) {
                                    callMethod.EditString(
                                            "BasketItemView",
                                            String.valueOf(gridLayoutManager.findFirstVisibleItemPosition())
                                    );
                                }
                            }
                        });
                    ...
                    public void onError(Exception e) {
                        callMethod.Log(e.getMessage());
                        callMethod.showToast("خطا در دریافت اطلاعات سبد خرید");
                    }
                }
        );
    }
    ...
    private void loadBasketSummary() {
        ...
        binding.bBasketATotalAmountBuy.setText(
                NumberFunctions.PerisanNumber(
                        broker_dbh.getFactorSumAmount(PreFac)
                )
        );

        binding.bBasketATotalCustomerBuy.setText(
                NumberFunctions.PerisanNumber(
                        broker_dbh.getFactorCustomer(PreFac)
                )
        );

        binding.bBasketATotalRowBuy.setText(
                NumberFunctions.PerisanNumber(
